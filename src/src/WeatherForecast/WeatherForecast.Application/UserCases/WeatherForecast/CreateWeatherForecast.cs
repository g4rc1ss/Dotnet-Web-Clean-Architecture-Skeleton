...
internal class CreateWeatherForecast(IWeatherForecastCommandCreateContract weatherForecastCommandCreate, IDataProtectionProvider dataProtection)
: ICreateWeatherForecastContract
{
    ...
    public async Task<WeatherForecastCommandCreateResponse> ExecuteAsync(WeatherForecastCommandCreateRequest weather, CancellationToken cancellationToken = default)
    {
        ...
        var createWeatherForecast = await weatherForecastCommandCreate.ExecuteAsync(weather, cancellationToken);
